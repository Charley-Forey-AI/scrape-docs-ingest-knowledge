#!/usr/bin/env python3
"""
Bulk-ingest Spectrum markdown files into Trimble Agent Studio Knowledge ingestion service.
...
Example:
  data/spectrum/hr-and-payroll/payroll/payments/foo.md
...
DEFAULT_ROOT_DIR = Path("data/spectrum")
DEFAULT_STATE_FILE = Path("scripts/.spectrum_ingest_state.json")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Do not poll jobs to terminal state; mark file as submitted after upload.",
    )
...
def main() -> int:
    ...
    app_name = env_or_file(env_values, "TRIMBLE_APP_NAME", "spectrum-docs-ingest") or "spectrum-docs-ingest"
    ...
    print(f"Wait for completion: {not args.no_wait_for_completion}")
    ...
    for idx, file_path in enumerate(files, start=1):
        ...
        if (not args.force_reprocess) and existing_status in {"completed", "submitted"}:
            skipped += 1
            print(f"[{idx}/{len(files)}] skip {existing_status}: {rel_key}")
            continue
    ...
    def process_file(idx: int, file_path: Path) -> str:
        rel_path = file_path.relative_to(root_dir)
        rel_key = rel_path.as_posix()
