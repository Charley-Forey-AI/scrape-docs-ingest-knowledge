...
# Create Earnings and Deduction Codes for Long Service Leave: Normal Termination, Accrual Prior to 18 Aug 1993
...
- From the
 ...
 Rate / Amount #1
 field. For the current rate, refer to the ATO website.
...
Note: During processing, the system calculates the tax based on
 all accrual dates that occur between 16 August 1978 and 17 August 1993 using the
 rate specified for the deduction code. The system will not calculate tax for leave
 accrued prior to 16 August 1978.If the payment amount is under the amount
 specified in the Leave
 Flat Rate Limit
 field (Limits and Rates form), the system does not use
 the rate specified for the deduction code but instead withholds the lesser of:
