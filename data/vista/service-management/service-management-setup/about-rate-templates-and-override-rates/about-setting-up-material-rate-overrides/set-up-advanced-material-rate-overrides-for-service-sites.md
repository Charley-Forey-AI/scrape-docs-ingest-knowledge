---
title: "Set up Advanced Material Rate Overrides for Service Sites | Trimble Help"
source_url: "https://help.trimble.com/en/vista/vista/service-management/service-management-setup/about-rate-templates-and-override-rates/about-setting-up-material-rate-overrides/set-up-advanced-material-rate-overrides-for-service-sites"
fetched_at: "2026-03-25T16:24:33.094126+00:00"
menu_path: "/en/vista/vista/service-management/service-management-setup/about-rate-templates-and-override-rates/about-setting-up-material-rate-overrides/set-up-advanced-material-rate-overrides-for-service-sites"
---

# Set up Advanced Material Rate Overrides for Service Sites

You can set up base rate overrides for materials at the service site level using the SM Rate Override Base form (accessed via SM Service Sites). You can also set up advanced overrides for materials by rate break point or category/material using the SM Rate Override Material (accessed via SM Rate Override Base).
 You will only need to set up material rate overrides by service site if they differ from the base and override rates defined for the rate template associated with the service site.The following instructions guide you through setting up overrides to the base rate, as well as overrides by break point and category or material.

1. Open the SM Service Sites form.

1. In the Service Site field, enter the service site for which to set up override material rates.

1. Select the Override Rates button (below Rate Template field).The SM Rate Override Base form displays.

1. In the Material Rates section, use the Type drop-down to select M-Markup or D-Discount as the material rate type. Select the N-No Overrides option if you do not want to override the base rate defined for the service site's rate template. This will disable the remaining fields in the Material Rates section; however, you can still set up advanced material rate overrides. See Steps 10 and 11 below.
...
1. In the Break Point Value field, enter the value at which to begin discounting or marking up materials. Depending on the Break Points By option you selected for the service site (in SM Rate Override Base), this amount will represent either Total Cost or Unit Cost. For example, if you selected the Total Cost option and you provide a larger discount when a material's total cost exceeds $500, you would enter 500.00 in this field.Note: If you selected the N-No Overrides rate type option, the system uses the Break Point By value selected for the service site's rate template to determine whether this amount represents Total Cost or Unit Cost.
