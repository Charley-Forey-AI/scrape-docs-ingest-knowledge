---
title: "Journal Entry - Cost Center Information | Trimble Help"
source_url: "https://help.trimble.com/en/spectrum/spectrum/accounting/general-ledger/spectrum-menus/data-entry-overview/journal-entry/journal-entry---cost-center-information"
fetched_at: "2026-04-03T20:05:26.860879+00:00"
menu_path: "/en/spectrum/spectrum/accounting/general-ledger/spectrum-menus/data-entry-overview/journal-entry/journal-entry---cost-center-information"
---

# Journal Entry - Cost Center Information

The Journal Entry screen will exclude journal entries from the list box if the operator does not have security authorization to the cost center specified in the journal entry header. Spectrum verifies the cost center assigned to the G/L account code by comparing the entry to the list of allowed cost centers for that G/L account. Spectrum compares the G/L account's list of shared cost centers with cost centers in the operator's assigned scheme, and if there are no common cost centers, then that G/L account code is not allowed. Validation is also performed if the operator attempts to change or delete an existing distribution line. Below is a chart that outlines the impact of cost centers on different types of distribution lines.
...
Direct Work Order
For direct work orders, the cost center is assigned based on the cost center associated with the work order. After Spectrum finds the cost center assigned to the work order, it will validate whether or not the operator has security authorization for that cost center--including any operator scheme overrides for work order.
When you change the work order field in the Edit Journal Entry Detail sub-window, Spectrum automatically adjusts the cost center assigned to the journal entry line, based on the cost center in the Work Order header table. Security authorization for the new work order and cost center are re-evaluated, and you cannot enter the new work order if you do not have permission for the new cost center.
If the new entry is disallowed, then the work order and cost center entries will revert to previous settings in the detail window (even if no longer authorized).
The cost center field will change to display only for direct work order cost entries in the New or Edit Journal Entry Detail sub-window.
